...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Log inicial sem expor email completo
    console.log(`[API] === PROCESSANDO BIGPOINTS ===`);
    console.log(`[API] Email: ${maskEmail(email)}`);
    ...
    if (decodedToken.email !== email) {
      console.log(`[API] ERRO: Email não corresponde - Token: ${maskEmail(decodedToken.email)}, Enviado: ${maskEmail(email)}`);
    ...
    if (now - lastCall < RATE_LIMIT_MS) {
      ...
      return res.status(429).json({
        success: false,
        message: `Muitas requisições. Aguarde ${remaining} segundos e tente novamente.`
      });
    }
    rateLimitCache.set(userId, now);

    console.log(`[API] UID: ${userId} (Email: ${maskEmail(email)})`);
    ...
    if (existingDoc.exists) {
      ...
      console.log(`[API] ✅ BIG Points atualizados para UID ${userId} em ${date}: ${currentAmount} → ${newAmount}`);
      ...
      console.log(`[API] ✅ Novo registro criado para UID ${userId} em ${date}: ${numericAmount}`);
...
/**
 * Mascara email para exibição segura em logs.
 * Exemplo: "usuario@dominio.com" → "us***@dominio.com"
 */
